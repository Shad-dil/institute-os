  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1.5">
        ...
        />
        {errors.courseId && (
          <p className="text-xs text-red-600">{errors.courseId.message}</p>
        )}
        {selectedCourse && (
          <p className="text-xs text-slate-400">
            Standard fee: ₹{selectedCourse.fees.toLocaleString("en-IN")} ·{" "}
          ...
          <Controller
            ...
            render={({ field }) => (
              <Select
                value={field.value}
                onValueChange={field.onChange}
                items={selectedCourse.batches.map((c) => ({
                  value: c.id,
                  label: c.name,
                }))}
              >
      ...
      <div className="rounded-lg border border-slate-100 bg-slate-50 p-4">
        <p className="text-sm font-medium text-slate-700">
          Advance Payment Collected Today
        </p>
        ...
        {advanceAmount > 0 && (
          <p className="mt-3 text-sm text-slate-600">
            ...
            <span className="font-semibold text-slate-900">
              ₹{balanceAfterAdvance.toLocaleString("en-IN")}
            </span>
          </p>
        )}
      </div>
